...
def computeHomography(f1, f2, matches, A_out=None):
    ...
    idx_min_ev = Vt.shape[0] - 1
    H = Vt[idx_min_ev].reshape(3, 3)
...
def getInliers(f1, f2, matches, M, RANSACthresh):
    ...
    for i in range(len(matches)):
        ...
        f1_to_f2 = np.dot(M, f1_point)
        f2x_pred = f1_to_f2[0] / f1_to_f2[2]
        f2y_pred = f1_to_f2[1] / f1_to_f2[2]

        diff_sum = (b_x - f2x_pred)**2 + (b_y - f2y_pred)**2
...
def leastSquaresFit(f1, f2, matches, m, inlier_indices):
    ...
    if m == eTranslate:
        ...
        inliers_matches = []
        for i in range(len(inlier_indices)):
            inliers_matches.append(matches[inlier_indices[i]])

        M = computeHomography(f1, f2, inliers_matches)
