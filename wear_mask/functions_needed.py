...
def fitEllipse(x,y):
    x = x[:,np.newaxis]   #将shape(n,)的列表 shape(n,1)
    y = y[:,np.newaxis]
...
def color_the_mask(mask_image1, color, intensity):
    assert 0 <= intensity <= 1, "intensity should be between 0 and 1"
    RGB_color = ImageColor.getcolor(color, "RGB")    #把颜色转化为RGB tuple
    RGB_color = (RGB_color[2], RGB_color[1], RGB_color[0])
    # print(RGB_color)
    orig_shape = mask_image1.shape
    # print(orig_shape)
    bit_mask = mask_image1[:, :, 3]
    mask_image1 = mask_image1[:, :, 0:3]

    color_image = np.full(mask_image1.shape, RGB_color, np.uint8)
    mask_color = cv2.addWeighted(mask_image1, 1 - intensity, color_image, intensity, 0)
...
def texture_the_mask(mask_image2, texture_path, intensity):
    assert 0 <= intensity <= 1, "intensity should be between 0 and 1"
    orig_shape = mask_image2.shape
    bit_mask = mask_image2[:, :, 3]
    mask_image2 = mask_image2[:, :, 0:3]
    ...
    mask_texture = cv2.addWeighted(
        mask_image2, 1 - intensity, texture_image, intensity, 0
    )
...
def mask_image(img_path, args):
    ...
    if verbose:
        tqdm.write("检测到人脸数: {:2d}".format(len(face_locations)))
